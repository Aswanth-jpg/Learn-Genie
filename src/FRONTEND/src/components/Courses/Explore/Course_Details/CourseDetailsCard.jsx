...
const CourseDetailsCard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://learngenieapi-production.up.railway.app/api/courses?ID=${courseId}`
      )
      .then((response) => {
      ...
      <div className="card-details-container">
        {
          ...
          <div className="card-square-left">
            ...
            <div className="left-btn">
              {course.course_url && (
                <a className="course_url" href={course.course_url} target="_blank" rel="noopener noreferrer">
                  <button className="button-55" role="button">
                    {course.platform === 'Coursera' ? 'Enroll on Coursera' : 'Go to Course'}
                  </button>
                </a>
              )}
            </div>
